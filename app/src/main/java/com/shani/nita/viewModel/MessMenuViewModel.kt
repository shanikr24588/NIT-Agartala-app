...
class MessMenuViewModel: ViewModel() {
    ...
    private val _messmenuMap = MutableLiveData<MutableMap<String, List<MessMenuModel>>>()
    val messmenuMap:LiveData<MutableMap<String, List<MessMenuModel>>> = _messmenuMap
        ...
        messmenuRef.document(hostel)
            ...
            .addOnSuccessListener { querySnapshot ->
                ...
                for (document in querySnapshot.documents) {
                    val item = document.toObject(MessMenuModel::class.java)
                    if (item != null) {
                        list.add(item)
                    }
                }
                // Update map and notify observers
                _messmenuMap.value = _messmenuMap.value?.apply { put(day, list) } ?: mutableMapOf(day to list)
                _messmenuList.postValue(list)
                _loading.postValue(false)
            }
            .addOnFailureListener {
                _loading.postValue(false) // Hide loading indicator on failure
                Log.e(TAG, "Error getting mess menu for $day", it)
            }
    }
